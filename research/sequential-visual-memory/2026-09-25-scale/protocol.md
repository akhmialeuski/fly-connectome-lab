...
- **F1, blank windows (development and confirmation).** Accuracy is reported at each blank delay. The rule is paired. Take the central-brain accuracy after 16 blank windows minus the accuracy at delay 0 on the same held-out photographs, with its 95% identity-cluster bootstrap interval (10,000 draws). F1 holds on the confirmation cohort if that interval lies inside (−5, +5) points. The reason is structural. The network is deterministic and noise-free, and near the zero state a blank window applies the linear map $x \mapsto \left((1-\lambda) I + \lambda g W\right) x$ four times. Its eigenvalues $0.98 + 0.02\, g \mu$ stay near 1 for every eigenvalue $\mu$ of $W$ with $|\mu| \le 1$. The map is therefore invertible and contracts slowly. The readout is refitted and rescaled at each delay, so it can undo a uniform shrinkage. Only the fast driven neurons lose their own state, and they are replaced every update anyway. An interval reaching beyond −5 points would contradict this argument.
- **F2, interference (development seed 0 and confirmation).** After the 16th glimpse, the network sees the first 1, 2, 4, 8 or 16 glimpses of a partner photograph. The partner is a photograph of another identity from the same split, chosen by a seeded permutation (`interference_partners`), so a held-out photograph never enters a training state. The readout still has to name the original identity. Accuracy is reported against the number of interfering glimpses. The half-retention is the number of glimpses at which accuracy above chance falls to half its value without interference, found by linear interpolation and reported as more than 16 if it never falls that far. This is the forgetting measurement proper. It extends T34's label-free window recall to identity decoding. F2 is descriptive and has no pass or fail threshold. The paired change after 16 interfering glimpses is reported with its identity-cluster interval.

**Linear time scale, for reference.** The slowest recurrent mode decays by $(0.98 + 0.02 \cdot 0.737)^4 \approx 0.979$ per window, a time constant of about 47 windows. Under F1 this bounds the amplitude decay, not the information. No threshold is set for F2. At this operating point T34 found early glimpses still recallable after 15 later ones (R² 0.35 against 0.41 for the last window), so the half-retention may exceed 16 glimpses.
...
- Numba threads: 12. Estimated time: about 25 minutes per recording with 16 extra windows (persistent and interference), about 13 minutes per reset recording, and roughly 6 to 7 hours in all, including evaluation.
...
**Second amendment, 2026-09-25, before any T38 recording.** The first amendment compared single accuracy values under F1. F1 now uses a paired identity-cluster interval for delay 16 minus delay 0. The expectation that interference acts within a few glimpses is withdrawn, because it contradicted T34's recall measurements, and F2 is descriptive. The time estimate now includes the interference recordings. Both points were raised while the T38 knowledge-base report was being updated.

**Third amendment, 2026-09-26, after the first three recordings and before any score was read.** The first development evaluation (encoder seed 0) failed. One readout, which the order of saved models identifies as `reset/central_brain`, exhausted the full T35 budget of 50,000 L-BFGS iterations without reaching the 1e-6 tolerance, although the continuation fix had extended it three times. With 100 classes, 1,400 training photographs and PCA features that are not variance-normalized, the problem is ill-conditioned and converges slowly. The budget is raised to **500,000 iterations** for every T38 fit (`--max-iterations 500000`, recorded in each evaluation's parameters). The objective, the tolerance, the C grid and the solver are unchanged, so every fit that converges reaches the same unique optimum. A fit that still does not converge remains a failure. The failed attempt is preserved, and no score from it was inspected. The recordings from `1508422` stand, and evaluation runs from the amended commit.
